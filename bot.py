...
def start(m):
        ...
        bot.send_message(m.chat.id, 'Авторизуйтесь!Авторизовываясь вы соглошаетесь с политикой конфедециальности',reply_markup=keyboard)
...
def callback_worker(call):
    ...
    if call.data == 'dop_ob':
        keyboard = types.InlineKeyboardMarkup()
        # По очереди готовим текст и обработчик для каждого знака зодиака
        key_ove1n = types.InlineKeyboardButton(text='Перейти в магазин',
    ...
    if call.data =='avto_podbor':
        ...
        key_oven12 = types.InlineKeyboardButton(text='Связаться со специалистом', url='https://t.me/sashakhasanova')
        keyboard.add(key_oven12)
        bot.send_message(call.message.chat.id,'Автоподбор - это услуга, с помощью которой, наши специалисты помогут выбрать и купить автомобиль с пробегом по вашим параметрам',reply_markup=keyboard)

    if call.data =='prod_avto':
        def get_surname1(message):
            global surname1
            surname1 = message.text
            bot.send_message(call.message.chat.id, 'Какой у вашей машины пробег')
            bot.register_next_step_handler(message, get_age1)

        def get_name1(message):  # получаем фамилию
            global name1
            name1 = message.text
            bot.send_message(message.chat.id, 'Какой год выпуска у вашей машины')
            bot.register_next_step_handler(message, get_surname1)

        bot.send_message(call.message.chat.id, "Какая марка автомобиля?")
        bot.register_next_step_handler(call.message, get_name1)  # следующий шаг – функция get_name

        def get_age1(message):
            global age1
            # проверяем что возраст изменился

            age1 = message.text  # проверяем, что возраст введен корректно
            ...
            if len(str(age1)) <= 13:
                ...
                keyboard.add(key_no)
                question = 'Ваш пробег ' + str(age1) + ', год выпуска  ' + str(surname1) + ',марка ' + name1 + '?'
                bot.send_message(call.message.chat.id, text=question, reply_markup=keyboard)
            else:
                keyboard = types.InlineKeyboardMarkup()
                key_oven23 = types.InlineKeyboardButton(text='Вернуться в основное меню',
                ...
                bot.send_message(call.message.chat.id, 'Данные введены некоректно', reply_markup=keyboard)
    if call.data == "yes1":
        global surname1,name1,age1 # call.data это callback_data, которую мы указали при объявлении кнопки
        # код сохранения данных, или их обработки
        keyboard = types.InlineKeyboardMarkup()
        key_oven111111111111 = types.InlineKeyboardButton(text='Вернуться в основное меню', callback_data='v_menu')
        # И добавляем кнопку на экран
        ...
        device.publish('amk_avtoset1/kredit','пробег ' + str(age1) + ', год выпуска  ' + str(surname1) + ',марка ' + name1)
    elif call.data == "no1":
        keyboard = types.InlineKeyboardMarkup()
        key_oven111111111111 = types.InlineKeyboardButton(text='Вернуться в основное меню', callback_data='v_menu')
        # И добавляем кнопку на экран
        keyboard.add(key_oven111111111111)
        bot.send_message(call.message.chat.id,'заявка на продажу авто отменена',reply_markup=keyboard)
